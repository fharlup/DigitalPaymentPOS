<div x-data class="relative bg-gray-50 min-h-screen font-sans pb-24">

    {{-- ======================================================================== --}}
    {{-- A. BAR STATUS PESANAN (STICKY & REALTIME SYNC)                           --}}
    {{-- ======================================================================== --}}
    @if (count($activeTransactions) > 0)
        
        {{-- Container Sticky dengan Z-Index Tinggi --}}
        <div x-data="{ open: true }" 
            ...
            {{-- 1. Header Bar (Selalu Muncul) --}}
            <div class="flex justify-between items-center px-4 py-3 cursor-pointer h-[54px]" 
                 @click="open = !open">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex flex-col justify-center">
                        ...
                        {{-- Info singkat saat collapsed --}}
                        <span x-show="!open" x-transition class="text-[9px] text-gray-500 font-bold mt-1 leading-none">
            ...
            {{-- 2. Detail Kartu Transaksi (Isi Polling) --}}
            <div x-show="open" 
                 x-collapse 
                 class="bg-white border-t border-orange-100 shadow-inner max-h-[60vh] overflow-y-auto">
                 
                 {{-- WIRE:POLL DISINI: Mengecek status ke server setiap 3 detik --}}
                    ...
                    @foreach ($activeTransactions as $trx)
                        <div class="bg-white rounded-xl p-4 shadow-[0_2px_8px_rgba(0,0,0,0.08)] border border-gray-100 relative">
                            ...
                            <div class="flex gap-4">
                                {{-- Icon Status Berubah Warna --}}
                                <div class="shrink-0 pt-1">
                                    @if ($trx->status == 'paid')
                                        {{-- Icon Hijau (Lunas/Dimasak) --}}
                                        <div class="w-10 h-10 bg-green-100 text-green-600 rounded-full flex items-center justify-center border border-green-200 shadow-sm animate-pulse">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                        </div>
                                    @else
                                        {{-- Icon Kuning (Menunggu Bayar) --}}
                                        <div class="w-10 h-10 bg-yellow-50 text-yellow-600 rounded-full flex items-center justify-center border border-yellow-100">
                                ...
                                <div class="flex-1">
                                    ...
                                    @elseif($trx->metode_pembayaran == 'qris' && $trx->snap_token)
                                        <button onclick="snap.pay('{{ $trx->snap_token }}')" 
                                            ...
                                            <span>📱 Buka QRIS Lagi</span>
                                        </button>
                                    @else
                                        <div class="text-[11px] text-yellow-700 font-bold bg-yellow-50 px-3 py-2 rounded-lg border border-yellow-100">
    ...
    {{-- ======================================================================== --}}
    <div class="sticky z-40 bg-white shadow-sm transition-all duration-300 {{ count($activeTransactions) > 0 ? 'top-[54px]' : 'top-0' }}">
    ...
    <div class="p-4 max-w-[480px] mx-auto">
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 mb-8 space-y-4">
            ...
            <div>
                ...
                <input type="text" wire:model.blur="nama_pelanggan" placeholder="Cth: Budi Santoso" 
                       class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm font-bold text-gray-800 focus:outline-none focus:border-orange-500 focus:bg-white transition-all @error('nama_pelanggan') border-red-500 bg-red-50 @enderror">
        ...
        @foreach ($kategoris as $kategori)
            <div id="kategori-{{ $kategori->id }}" class="scroll-mt-40 mb-8">
    ...
    {{-- ======================================================================== --}}
    {{-- D. FLOATING BUTTON & MODAL UI                                            --}}
    {{-- ======================================================================== --}}

    {{-- 1. Floating Button Keranjang --}}
    @if (!empty($cart) && !$showCartModal)
        <div class="fixed bottom-0 left-0 right-0 z-50 bg-yellow-400 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.1)] cursor-pointer animate-slide-up" 
            ...
            <div class="bg-white w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl relative z-10 animate-bounce-in flex flex-col max-h-[90vh]">
                ...
                <div class="p-6 flex-1 overflow-y-auto">
                    <div class="flex justify-between items-start mb-2">
                        ...
                        <span class="bg-orange-100 text-orange-700 text-xs font-bold px-2 py-1 rounded-md border border-orange-200">Stok: {{ $selectedProduct->stok }}</span>
                    </div>
                ...
                <div class="p-4 border-t border-gray-100 bg-gray-50">
                    ...
                    @else 
                        <button disabled class="w-full bg-gray-300 text-gray-500 py-4 rounded-xl font-bold text-lg cursor-not-allowed">MAAF STOK HABIS</button> 
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="fixed inset-0 z-[100] flex items-end justify-center sm:items-center">
            ...
            <div class="bg-white w-full max-w-[480px] rounded-t-3xl sm:rounded-3xl p-6 relative z-10 max-h-[85vh] flex flex-col shadow-2xl animate-slide-up">
                ...
                <div class="flex justify-between items-center mb-6 border-b border-gray-100 pb-4">
                    ...
                    <button wire:click="$set('showCartModal', false)" class="p-2 bg-gray-50 rounded-full hover:bg-gray-100 text-gray-500"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
                ...
                <div class="border-t border-gray-100 pt-4 space-y-4 bg-white">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <button wire:click="checkout('qris')" wire:loading.attr="disabled" 
                                class="bg-blue-600 hover:bg-blue-700 text-white py-3.5 rounded-xl font-bold text-sm uppercase flex justify-center items-center gap-2 shadow-md active:scale-95 transition-all">
